'use strict'
var rpgcoin = require('rpgcoinjs-lib');
...
var decodeInput = function(tx){
    var result = tx.ins.map(function(input, n){
        var vin = {
            ...
            n : input.index,
            script: rpgcoin.script.toASM(input.script),
...
var decodeOutput = function(tx, network){
    ...
    var format = function(out, n, network){
        var vout = {
            ...
            n: n,
            scriptPubKey: {
                asm: rpgcoin.script.toASM(out.script),
                hex: out.script.toString('hex'),
                type: rpgcoin.script.classifyOutput(out.script),
                addresses: [],
            },
        };
        ...
        case 'pubkeyhash':
        case 'scripthash':
            vout.scriptPubKey.addresses.push(rpgcoin.address.fromOutputScript(out.script, network));
            break;
        case 'witnesspubkeyhash': // NATIVE SEGWIT
            break;
        case 'pubkey': // There really is no address
            var pubKeyBuffer = rpgcoin.script.pubKey.output.decode(out.script);
            vout.scriptPubKey.addresses.push(rpgcoin.ECPair.fromPublicKeyBuffer(pubKeyBuffer, network).getAddress());
            break;
...
var TxDecoder = module.exports = function(rawtx, network){
    ...
    this.tx = rpgcoin.Transaction.fromHex(rawtx);
